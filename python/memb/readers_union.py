...
class ReadersUnion(BaseReader):
    def __init__(self, readers, mode):
        ...
        self.union_maker = UNION_MAKERS.get(mode)
        
        if self.union_maker is None:
            raise KeyError('Mode {} is not supported. Available modes are {}'.format(
                mode, list(UNION_MAKERS.keys())))

        self.union_maker.check(readers)

    @property
    def dim(self):
        return self.union_maker.dim([reader.dim for reader in self._readers])

    def word_embedding(self, word):
        return self.union_maker.merge([reader.word_embedding(word) for reader in self._readers])

    def batch_embedding(self, words):
        return self.union_maker.merge([reader.batch_embedding(words) for reader in self._readers])

    def tokenizer_embedding(self, tokenizer):
        return self.union_maker.merge([reader.tokenizer_embedding(tokenizer) for reader in self._readers])
